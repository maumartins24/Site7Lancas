<template>
  <div class="min-h-screen">
    ...
    <header
      ...
      :class="scrolled
        ? 'bg-white/95 backdrop-blur-lg border-b border-brand-400/20 shadow-sm'
        : 'bg-transparent'"
    >
      <!-- Faixa kente -->
      ...
      <div class="max-w-7xl mx-auto px-5 md:px-8 flex items-center justify-between h-16 md:h-[68px]">
        ...
        <button @click="goTo('inicio')" class="flex items-center gap-3 group shrink-0">
          <div class="w-10 h-10 md:w-11 md:h-11 rounded-full bg-brand-100 border border-brand-400/40 flex items-center justify-center group-hover:border-brand-400 transition-colors duration-300">
            <span class="text-brand-400 font-serif font-black text-base">⚔</span>
          </div>
          <div class="hidden sm:block leading-tight">
            <p class="font-serif font-bold text-brand-400 text-sm md:text-base tracking-tight">Ogum 7 Lanças</p>
            <p class="font-mono text-[9px] uppercase tracking-widest text-brand-400">Templo Umbandista</p>
          </div>
        </button>
      ...
      <Transition name="slide-down">
        <div v-if="mobileOpen" class="lg:hidden bg-white/98 backdrop-blur-xl border-t border-brand-400/15">
    ...
    <!-- FOOTER -->
    <footer class="bg-white border-t border-brand-200 text-brand-600">
      <div class="kente-bar w-full" />
      <div class="max-w-7xl mx-auto px-6 md:px-10 py-14 grid grid-cols-1 md:grid-cols-3 gap-10">
        <div class="space-y-4">
          <h3 class="font-serif text-xl text-brand-800">Terreiro Ogum 7 Lanças</h3>
          <p class="font-mono text-[9px] uppercase tracking-widest text-brand-400">Templo Umbandista</p>
          <p class="text-sm text-brand-600 leading-relaxed">Umbanda, cultura negra de terreiro.<br>Axé · Fé · Comunidade.</p>
        ...
        <div class="space-y-2">
          <p class="font-mono text-[9px] uppercase tracking-widest text-brand-400 mb-4">Navegação</p>
          <button v-for="item in allItems" :key="item.id"
            @click="goTo(item.id)"
            class="block text-sm text-brand-600 hover:text-brand-400 transition-colors duration-200 text-left">
            {{ item.label }}
          </button>
        </div>
        <div class="space-y-3">
          <p class="font-mono text-[9px] uppercase tracking-widest text-brand-400 mb-4">Contato</p>
          <p class="text-sm text-brand-600">📍 Av. Itaquera, 1102 – Parque Maria Luiza</p>
          <p class="text-sm text-brand-600">✉️ terreiroogumsetelancas@gmail.com</p>
          <a href="https://www.instagram.com/terreiro7lancas/" target="_blank" rel="noopener noreferrer"
            class="block text-sm text-brand-600 hover:text-brand-400 transition-colors duration-200">
            📷 @terreiro7lancas
          </a>
          <div class="pt-1 w-full h-32 border border-brand-200 overflow-hidden">
...
/* Nav link desktop */
.nav-link {
  @apply font-mono text-[10px] uppercase tracking-widest px-4 py-2 text-brand-600 cursor-pointer border-b-2 border-transparent transition-all duration-200 hover:text-brand-400 bg-transparent;
}
.nav-link.active { @apply text-brand-400 border-brand-400; }

/* Botão apoiar */
.btn-apoiar {
  @apply font-mono text-[10px] uppercase tracking-widest px-5 py-2.5 bg-brand-400 text-white border-none cursor-pointer transition-all duration-200 hover:bg-brand-600 font-medium;
  clip-path: polygon(8px 0%, 100% 0%, calc(100% - 8px) 100%, 0% 100%);
}

/* Ícone btn */
.icon-btn {
  @apply w-8 h-8 flex items-center justify-center text-brand-600 hover:text-brand-400 transition-colors duration-200 cursor-pointer bg-transparent border-none;
}

/* Mobile nav */
.mobile-nav-link {
  @apply flex items-center gap-3 font-mono text-[10px] uppercase tracking-widest text-brand-600 py-3 px-2 border-b border-brand-200 hover:text-brand-400 transition-colors duration-200 cursor-pointer bg-transparent border-l-0 border-r-0 border-t-0 w-full text-left;
}
.mobile-nav-link.active { @apply text-brand-400; }
